...
def load_data():
    try:
        ...
        metadata = pd.read_csv(os.path.join(data_dir, 'p1_model_metadata.csv'))
...
with st.sidebar:
    ...
    if data_loaded and metadata is not None:
        st.metric("Architecture", f"{int(metadata['n_layers'].values[0])} Layers x 64 Neurons")
        ...
        st.markdown(f"- Input Angle: {INPUT_ANGLE} deg")
    else:
...
with tab1:
    ...
    with col1:
        ...
        st.info(f"**Design Goal:** Diffract 550 nm light to **{target_angle} deg**")

        with st.expander("Physics Background"):
            st.latex(r"n(\sin\theta_{out} - \sin\theta_{in}) = \frac{m \cdot \lambda}{P}")
            st.markdown("**n** = 1.5 | **theta_in** = 0 deg | **m** = -1 | **lambda** = 550 nm")
...
with tab2:
    ...
    if data_loaded and predictions is not None and training_history is not None:
        ...
        if metadata is not None:
            ...
            with col4:
                st.metric("R2 Score", f"{metadata['r2_score'].values[0]:.6f}")
...
with tab3:
    ...
    if data_loaded and doe_results is not None:
        ...
        # Factor tabs
        factor_tab1, factor_tab2, factor_tab3 = st.tabs(["Dataset Size", "Network Depth", "Epochs"])
