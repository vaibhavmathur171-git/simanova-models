...
def diffraction_angle(pitch_nm: float, wavelength_nm: float, theta_in: float = 0.0) -> float:
    sin_out = np.sin(np.radians(theta_in)) + wavelength_nm / pitch_nm
...
def generate_dataset(n_samples: int = 50000) -> pd.DataFrame:
    ...
    print(f"\nSaved to data/spectral_data.csv")
